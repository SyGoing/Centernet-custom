...
class MobileNetv2Det(nn.Module):
    def __init__(self,heads, head_conv,width_mult=1.,is_train=True):
        ...
        self.ups=[]
        for i in range(3):
            dcn=DCN(self.last_channel, self.last_channel,
...
def get_mv2dcnrelu_net(heads, head_conv,is_train):
  model =MobileNetv2Det(heads, head_conv=head_conv,width_mult=1.,is_train=is_train)
  model.init_weights()
  return model
