...
class opts(object):
    def __init__(self):
        self.parser = argparse.ArgumentParser(description="Generate Batch Normalized model for inference")
        # basic experiment setting
        self.parser.add_argument('--task', type=str, default='ctfacedetlm',
                                 help='ctfacedet | ctdet | ctfacedetlm |ddd | multi_pose | exdet ')
        self.parser.add_argument('--dataset', default='voc_facedet_reglm', type=str,
                                 help='voc_det | coco_det |voc_facedet | coco_facedet |coco_facedet_hplm|voc_facedet_reglm')
        ...
        # model
        self.parser.add_argument('--arch', default='mv2relu',
        ...
        self.parser.add_argument('--lr_step', type=str, default='30,90',
        ...
        ###Add for kpts regression
        self.parser.add_argument('--kpts_weight', type=float, default=0.05,
                                 help='loss weight for five keypoints.')
        self.parser.add_argument('--kptreg', type=bool, default=True,
                                 help='loss weight for bounding box size.')

        # multi_pose
        self.parser.add_argument('--hp_weight', type=float, default=0.1,
    ...
    def update_dataset_info_and_set_heads(self, opt, dataset):
        ...
        elif opt.task == 'ctfacedetlm':
            opt.heads = {'hm': opt.num_classes,
                         'wh': 2 if not opt.cat_spec_wh else 2 * opt.num_classes}
            if opt.reg_offset:
                opt.heads.update({'reg': 2})
            if opt.kpts_reg:
                opt.heads.update({'kpts': 10})  # 关键点回归

        elif opt.task == 'multi_pose':
            # assert opt.dataset in ['coco_hp']
            opt.flip_idx = dataset.flip_idx
            opt.heads = {'hm': opt.num_classes, 'wh': 2, 'hps': 10}
            if opt.reg_offset:
                opt.heads.update({'reg': 2})
            if opt.hm_hp:
    ...
    def init(self, args=''):
        default_dataset_info = {
            'ctdet': {'default_resolution': [512, 512], 'num_classes': 80,
                      ...
                      'dataset': 'coco'},
            'ctfacedet': {'default_resolution': [640, 640], 'num_classes': 1,
