...
class CTDetFacekptsDataset(data.Dataset):
    num_classes = 1
    ...
    def __init__(self, opt, split,add_lm=True):
        super(CTDetFacekptsDataset, self).__init__()
        self.data_dir = os.path.join(opt.data_dir, 'wilderface')
        self.img_dir = os.path.join(self.data_dir, 'JPEGImages')  # images
        self.class_names = ('__background__', 'face')
        self.max_objs = 50
        self._valid_ids = np.arange(1, 2, dtype=np.int32)
        self.has_landmark=add_lm
        ...
        if self.split == 'val':
            image_sets_file = self.data_dir + "/ImageSets/Main/test.txt"
        elif self.split == 'train':
            image_sets_file = self.data_dir + "/ImageSets/Main/trainval.txt"

        self.images = CTDetFacekptsDataset._read_image_ids(image_sets_file)
    ...
    def _get_annotation_lm(self, image_id):
        annotation_file = self.data_dir + f"/Annotations/{image_id}.xml"
        objects = ET.parse(annotation_file).findall("object")
        items=[]
        for object in objects:
            ...
            if class_name in self.class_names:
                ...
                lm = []
                if object.find('has_lm') is not None:
                    has_lm = int(object.find('has_lm').text)
                    if has_lm == 1:
                        landmark = object.find('lm')
                        lm.append(float(landmark.find('x1').text))
                        lm.append(float(landmark.find('y1').text))
                        lm.append(float(landmark.find('x2').text))
                        lm.append(float(landmark.find('y2').text))
                        lm.append(float(landmark.find('x3').text))
                        lm.append(float(landmark.find('y3').text))
                        lm.append(float(landmark.find('x4').text))
                        lm.append(float(landmark.find('y4').text))
                        lm.append(float(landmark.find('x5').text))
                        lm.append(float(landmark.find('y5').text))
                    else:
                        for i in range(10):
                            lm.append(0)
                else:
            ...
            if h > 0 and w > 0:
                ...
                if flag_lm > 1:
                    for idx in range(0, 10, 2):
                        lm[idx:idx + 2] = affine_transform(lm[idx:idx + 2], trans_output)
