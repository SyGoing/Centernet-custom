...
def _convert_softmax(node,graph,err):
    ...
    layer = myf("Softmax",name,[input_name],[output_name],in_place=inplace)
    # l_top_relu1 = L.ReLU(l_bottom, name=name, in_place=True)
...
def _convert_node_Split(node,graph,err):
    node_name = node.name
    input_name = str(node.inputs[0])
    output_name_list = [str(i) for i in node.outputs]
    axis = node.attrs.get("axis", 1)
    layer = myf('Slice', node_name, [input_name], output_name_list, axis = axis)

    if axis == 1:
        input_channels=graph.channel_dims[input_name]
        num_outputs=len(output_name_list)

        for name in output_name_list:
            graph.channel_dims[name] = input_channels/num_outputs
    return layer
